#!/usr/bin/env python3
"""v0.4 robust five-model MERFISH classifier (v0.31 + generalisable label features).
...
New in v0.4 (only leakage-free, dataset-transferable techniques)
----------------------------------------------------------------
* class-centroid cosine: cosine of each cell's row-centred expression to every
  class mean profile, built only from the fold's training rows with a
  leave-one-out correction for training cells (expression only -> transfers);
* expression-kNN true-label vote: soft class vote among each cell's nearest
  neighbours in PCA-expression space, using training-cell labels with self
  excluded (expression only -> transfers);
* Region x E/I x Segment bucket decode mask: restricts the argmax to the cell
  types actually seen for each anatomical bucket (most buckets are single-type),
  falling back to the E/I-allowed set, then to all classes.
These features feed the tree models (LGBM, CatBoost) and the linear models
(LR, MLP); the standalone KNN model stays on pure expression/QC features. The
same-experiment crutches from the source team (spatial neighbour composition,
per-section label priors, any external atlas) are deliberately NOT included.
...
Examples
--------
Validate on both group axes first (label features must hold up out-of-group):

    python src/v0.4-Robust-NoID-Centroid-KNNvote-BucketMask.py \
...
This script never uses external reference data.
"""
...
DEFAULT_OUT_DIR = ROOT / "outputs" / "v04_robust_noid_centroid_knn_bucket"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Defaults to <out-dir>/v0.4-Prediction-Robust5Mean.csv",
    )
    parser.add_argument(
    ...
    if args.submission_out is None:
        args.submission_out = (
            args.out_dir / "v0.4-Prediction-Robust5Mean.csv"
        )
    return args
...
# ============================ v0.4 additions (generalisable) ============================
# Three techniques learned from a strong leakage-free team (MrMitochondrion). All are
# fold-honest (reference = training rows only, leave-one-out for training rows) and use
# only expression / anatomical metadata, so they transfer to an unseen dataset.


def bucket_key(meta: pd.DataFrame) -> np.ndarray:
    """Region x E/I x Segment bucket.  Most buckets map to a single cell type, so this
    hard-constrains the label far more tightly than E/I alone, while staying purely
    anatomical (generalises to a new dataset)."""
    def norm(column: str) -> pd.Series:
        return meta[column].fillna("NA").astype(str).str.replace(r"\.0$", "", regex=True)

    return (
        norm("Region")
        + "|"
        + meta["Excitatory_vs_Inhibitory"].fillna("MISSING").astype(str)
        + "|"
        + norm("Segment")
    ).to_numpy()
...
def centroid_cosine(
    expression: np.ndarray, ref_pos: np.ndarray, y_ref_idx: np.ndarray, n_classes: int
) -> np.ndarray:
    """Cosine of each cell's row-centred expression against every class mean profile.
    Reference (labelled) rows use a leave-one-out centroid, so the feature never sees its
    own label.  Pure expression -> transfers to any dataset.  Returns (n_rows, n_classes)."""
    n, g = expression.shape
    sums = np.zeros((n_classes, g), np.float64)
    cnts = np.zeros(n_classes)
    np.add.at(sums, y_ref_idx, expression[ref_pos].astype(np.float64))
    np.add.at(cnts, y_ref_idx, 1)
    cnts = np.maximum(cnts, 1)
    Z = expression - expression.mean(1, keepdims=True)
    Zn = Z / np.maximum(np.linalg.norm(Z, axis=1, keepdims=True), 1e-9)
    cent = sums / cnts[:, None]
    Cz = cent - cent.mean(1, keepdims=True)
    Cz = Cz / np.maximum(np.linalg.norm(Cz, axis=1, keepdims=True), 1e-9)
    corr = (Zn @ Cz.T).astype(np.float32)
    own = np.full(n, -1)
    own[ref_pos] = y_ref_idx
    for c in range(n_classes):
        rows = np.where(own == c)[0]
        if len(rows) == 0 or cnts[c] <= 1:
            continue
        loo = (sums[c][None, :] - expression[rows]) / (cnts[c] - 1)
        L = loo - loo.mean(1, keepdims=True)
        L = L / np.maximum(np.linalg.norm(L, axis=1, keepdims=True), 1e-9)
        corr[rows, c] = (Zn[rows] * L).sum(1)
    return corr


def knn_expr_vote(
    expression: np.ndarray, ref_pos: np.ndarray, y_ref_idx: np.ndarray,
    n_classes: int, seed: int, ks: tuple[int, ...] = (15, 40),
) -> np.ndarray:
    """Soft class vote among each cell's nearest neighbours in PCA-expression space, using
    the *true* labels of the labelled reference rows (leave-one-out for reference rows).
    Pure expression -> transfers to any dataset.  Returns (n_rows, n_classes * len(ks))."""
    from scipy.spatial import cKDTree

    n = len(expression)
    n_comp = max(1, min(50, len(ref_pos) - 1, expression.shape[1]))
    projected = PCA(n_components=n_comp, random_state=seed).fit_transform(
        StandardScaler().fit_transform(expression)
    ).astype(np.float32)
    self_pos = np.full(n, -1)
    self_pos[ref_pos] = np.arange(len(ref_pos))
    kq = min(max(ks) + 1, len(ref_pos))
    _, ind = cKDTree(projected[ref_pos]).query(projected, k=kq)
    if ind.ndim == 1:
        ind = ind[:, None]
    keep = ind != self_pos[:, None]                       # drop the query row's own copy
    order = np.argsort(~keep, axis=1, kind="stable")
    ind_c = np.take_along_axis(ind, order, 1)
    navail = keep.sum(1)
    labels = y_ref_idx[ind_c]                             # (n, kq) neighbour class indices
    blocks = []
    for k in ks:
        take = np.arange(ind_c.shape[1])[None, :] < np.minimum(k, navail)[:, None]
        votes = np.zeros((n, n_classes), np.float32)
        for c in range(n_classes):
            votes[:, c] = ((labels == c) & take).sum(1)
        votes /= np.maximum(votes.sum(1, keepdims=True), 1e-9)
        blocks.append(votes)
    return np.hstack(blocks).astype(np.float32)


def label_features(
    expression: np.ndarray, train_rows: np.ndarray, y_train_idx: np.ndarray,
    n_classes: int, seed: int,
) -> np.ndarray:
    """Fold-honest label-derived features for every row (reference = train_rows):
    class-centroid cosine (n_classes) + expression-kNN class vote (n_classes * 2)."""
    corr = centroid_cosine(expression, train_rows, y_train_idx, n_classes)
    vote = knn_expr_vote(expression, train_rows, y_train_idx, n_classes, seed)
    return np.hstack([corr, vote]).astype(np.float32)


def allowed_by_bucket(
    y: np.ndarray, bucket: np.ndarray, train_rows: np.ndarray
) -> dict[str, set[str]]:
    out: defaultdict[str, set[str]] = defaultdict(set)
    for i in train_rows:
        out[bucket[i]].add(y[i])
    return dict(out)


def mask_by_bucket(
    probabilities: np.ndarray, classes: np.ndarray, bucket_rows: np.ndarray,
    ei_rows: np.ndarray, bucket_allowed: dict[str, set[str]],
    ei_allowed: dict[str, set[str]],
) -> np.ndarray:
    """Restrict argmax to the classes seen for each cell's Region x E/I x Segment bucket;
    fall back to the E/I-allowed set for buckets unseen in training, and to all classes
    when even the E/I value is unseen.  A bucket that removes all probability mass for a
    row is ignored for that row (unrestricted argmax)."""
    all_set = set(classes)
    allow = [
        (bucket_allowed.get(b) or ei_allowed.get(e) or all_set)
        for b, e in zip(bucket_rows, ei_rows)
    ]
    mask = np.array(
        [[label in group for label in classes] for group in allow], dtype=bool
    )
    masked = np.where(mask, probabilities, 0.0)
    row_sums = masked.sum(axis=1, keepdims=True)
    bad = (row_sums <= 0).ravel()
    if bad.any():
        masked[bad] = probabilities[bad]
        row_sums = masked.sum(axis=1, keepdims=True)
...
def five_model_probabilities(
    ...
    # v0.4: fold-honest label-derived features (class-centroid cosine + expression-kNN
    # true-label vote), with train_rows as the only reference (leave-one-out for train
    # rows). Fed to the tree models (LGBM, CatBoost) and the linear models (LR, MLP);
    # the standalone KNN model stays on pure expression/QC features for diversity.
    label_all = label_features(
        store.expression, train_rows, encoded_y, len(classes), seed
    )
    tree_train = np.hstack([static_train, label_all[train_rows]])
    tree_eval = np.hstack([static_eval, label_all[eval_rows]])
    linear_train = np.hstack([linear_train, label_all[train_rows]])
    linear_eval = np.hstack([linear_eval, label_all[eval_rows]])
    ...
    ).fit(tree_train, local_y)
    cat_probabilities = catboost.predict_proba(tree_eval)
...
def run_oof_scheme(
    ...
    bucket = bucket_key(bundle.meta_train)
    ...
    for fold, (train_rows, valid_rows) in enumerate(splits):
        ei_allowed = allowed_by_ei(y, ei, train_rows)
        bucket_allowed = allowed_by_bucket(y, bucket, train_rows)
        for model in BASE_MODELS:
            p_masked = mask_by_bucket(
                averaged[model][valid_rows], classes, bucket[valid_rows],
                ei[valid_rows], bucket_allowed, ei_allowed,
            )
        ...
        for name, target in [
            ...
            p_masked = mask_by_bucket(
                variants[name][valid_rows], classes, bucket[valid_rows],
                ei[valid_rows], bucket_allowed, ei_allowed,
            )
    ...
    artifact = args.out_dir / f"v04_oof_{scheme}.npz"
    np.savez_compressed(
        artifact,
...
def run_oof(args: argparse.Namespace, bundle: DataBundle) -> None:
    ...
    for scheme in args.schemes:
        ...
        pd.DataFrame(all_summary).to_csv(
            args.out_dir / "v04_oof_summary.csv", index=False
        )
        pd.DataFrame(all_folds).to_csv(
            args.out_dir / "v04_oof_folds.csv", index=False
        )
    summary_df = pd.DataFrame(all_summary)
    print(f"\n=== {VERSION} GROUPED OOF SUMMARY ===")
    print(summary_df.to_string(index=False))

    # Headline CV: pooled group-OOF accuracy of the primary model (5-model mean),
    # one line per group axis. This is the number to compare against v0.31.
    primary = summary_df[summary_df["model"] == "mean5"]
    print(f"\n=== {VERSION} CV (primary model = mean5) ===")
    for _, row in primary.iterrows():
        print(
            f"  {str(row['scheme']):<8} CV accuracy = {row['pooled_accuracy']:.4f}"
            f"   (fold mean {row['fold_mean']:.4f} +/- {row['fold_std']:.4f},"
...
def run_prediction(args: argparse.Namespace, bundle: DataBundle) -> None:
    ...
    ei_allowed = allowed_by_ei(y, train_ei, np.arange(n_train))
    bucket_allowed = allowed_by_bucket(
        y, bucket_key(bundle.meta_train), np.arange(n_train)
    )
    masked_mean5 = mask_by_bucket(
        variants["mean5"], classes, bucket_key(bundle.meta_test), test_ei,
        bucket_allowed, ei_allowed,
    )
    ...
    np.savez_compressed(
        args.out_dir / "v04_test_probabilities.npz",
        p_lgbm=averaged["lgbm"],
        p_lr=averaged["lr"],
        p_mlp=averaged["mlp"],
        p_knn=averaged["knn"],
...
def main() -> None:
    ...
    manifest = {
        ...
        "stacker": None,
        "label_features": [
            "class-centroid cosine (leave-one-out, expression only)",
            "expression-kNN true-label vote k=15,40 (leave-one-out)",
        ],
        "decode_mask": "Region x E/I x Segment bucket, E/I fallback, unrestricted fallback",
    ...
    (args.out_dir / "v04_manifest.json").write_text(
